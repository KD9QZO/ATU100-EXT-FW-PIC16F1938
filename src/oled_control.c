...
void oled_wr_str(char col, char page, char str[], char leng) { //    128*32 or 128*64 OLED
	...
	for (i = 0; i < leng; i++) { // write string
		...
		for (h = 0; h <= 4; h++) { // write letter
			...
			Soft_I2C_Write(w2.byte);
			if (led_type == 4 | led_type == 5) {
				Soft_I2C_Write(w2.byte); // 128*64
			}
		}
		Soft_I2C_Write(0);
		Soft_I2C_Write(0);
	...
	for (i = 0; i < leng; i++) { // write string
		if (str[i] == 0) g = 0;
		else g = str[i] - 32; // NULL detection
		for (h = 0; h <= 4; h++) { // write letter
			w1.byte = font5x8[g * 5 + h];
			if (page != 2 & page != 4 & page != 6) {
				w2.B7 = w1.B7;
			...
			Soft_I2C_Write(w2.byte);
			if (led_type == 4 | led_type == 5) Soft_I2C_Write(w2.byte); // 128*64
		}
		Soft_I2C_Write(0);
		Soft_I2C_Write(0);
